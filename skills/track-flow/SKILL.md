...
| **income-other** | Catch-all for positive-amount personal rows that fit no other source-type after every detection pass has run (phantom-paycheck, government-benefit, refund matching, windfall magnitude scan) | ✓ yes | Assign LAST, never by default — surface each income-other row to the user for confirmation, because "other" is where misclassifications hide. If the same kind of row recurs, that's signal for a new named source-type. |
