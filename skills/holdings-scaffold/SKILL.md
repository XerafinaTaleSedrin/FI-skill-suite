...
| `last_verified` | YYYY-MM-DD | yes | When the user last verified this balance. Written as `last-verified` in the holdings.md schema below (same kebab-case convention as every other field); distinct from the file-level `last-updated` frontmatter date. |
...
This format is the **cross-skill data contract**. Other skills (`/fi:fu-money-readout`, `/fi:crossover`, `/fi:redirect`, `/fi:wallchart`) read from holdings.md using this schema. Don't break it without coordinated updates across the suite.
...
| `/fi:wallchart` | Investment-accounts total | FI-threshold reference line (investment-accounts total × SWR ÷ 12; deliberately NOT net worth — non-investment assets aren't SWR-drawable) — holdings.md is a required input for its Step 1 |
