...
14. **Cross-reference report (private — not in the audit file).** Tell Marika which existing `fi-*` skills should add this book to their `sources:` frontmatter. The audit file stays clean of repo-mapping; the wiring lives in the SKILL.md files. If a load-bearing mechanic from this book doesn't have a `fi-*` home and earns one, propose the new skill (name + one-sentence purpose) — don't create it inside this skill's run; surface it as a recommendation.
