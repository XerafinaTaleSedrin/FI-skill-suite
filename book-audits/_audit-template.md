---
book: (Title)
author: (Author)
published: YYYY (publication year of edition reviewed)
audited: YYYY-MM-DD (when this audit was written)
auditor: (GitHub handle or "Marika Olson")
hearth_verdict: (one of: nap-worthy / hiss-worthy / windowsill-approved / would-knock-off-the-desk)
status: (draft | complete)
---

# Audit: [Book Title]

> Output of the `/fi:audit` skill. Strips persona, surfaces load-bearing mechanic, separates layers, delivers Hearth's verdict.

---

## 1. The premise in one paragraph (no jargon)

[2-4 sentences. What is this book actually about, in plain terms? No jargon, no marketing copy.]

---

## 2. The load-bearing mechanic — the 1-3 actual moves the book proposes

[Strip away the rhetorical packaging. What does the book actually want the reader to do? Should be 1-3 concrete moves.]

1. [Move 1]
2. [Move 2]
3. [Move 3]

---

## 3. What aged well (still applicable in 2026)

[Concepts and patterns that survive the era they were written in.]

- [Item 1] — why it's still load-bearing
- [Item 2] — why it's still load-bearing

---

## 4. What aged poorly (era-bound math, dead assumptions, vibes-based claims)

[Specific numerical assumptions, named tools, era-specific frames that no longer hold. Be blunt. Cite chapter or page where possible.]

- [Item 1] — why it's dated; how it would mislead a 2026 reader
- [Item 2] — why it's dated

---

## 5. What's missing (things the book ignores that matter now)

[What does a 2026 reader need to consider that the book doesn't address?]

- [Item 1] — e.g., AI-displacement risk for white-collar work
- [Item 2] — e.g., HSA / high-deductible health plans (post-ACA reality)
- [Item 3] — e.g., gig income / multi-employer / freelance reality

---

## 6. The honest verdict — who should read it, who should skip it

**Read it if**: [specific reader profile — e.g., "you're 25-35, US-based, currently feel financially overwhelmed, and have never tried a category-by-category spending audit"]

**Skip it if**: [specific reader profile — e.g., "you've already done holdings-scaffold + monthly-tabulation work; the rest of this book is rhetorical reinforcement of moves you've made"]

---

## 7. The 5-line takeaway you'd actually run

The smallest version of the book's advice. If a reader does only these five things and skips everything else in the book, do they get the load-bearing benefit?

1. [Line 1]
2. [Line 2]
3. [Line 3]
4. [Line 4]
5. [Line 5]

---

## 8. Hearth's verdict

> [One-line cat-take, written in Hearth's voice. Tonal range: nap-worthy / hiss-worthy / windowsill-approved / would-knock-off-the-desk.]

**Verdict**: [one of: nap-worthy / hiss-worthy / windowsill-approved / would-knock-off-the-desk]

---

## Citations

- [Page or chapter for every load-bearing claim referenced above.]

---

*Audit written by [auditor] on YYYY-MM-DD. Schema version: 1.0.*
