...
Example: *"Transaction aggregators consolidate bank, credit card, and brokerage transactions into a single view that the `/fi:track-flow` skill can import. Without an aggregator, the user falls back to manual capture from each financial institution's website."*
